...
$(function () {
     ...
     */
    $(".app-header__hasChild, .app-header__menu .menu-item-has-children").hover(function () {
    ...
    $('.js-form-project-submit').on('click', function(event){
        event.preventDefault();

        var $form = $(this).closest('form');
        var val = $(this).data('offer');
    ...
    $('.b-cabinet-info__user-date').on('keyup', function(){
        $(this).closest('.b-cabinet-info__item').addClass('is-changed');
    });

    // update user options
    function updateUserMaster(options) {
        var data = {
            action: 'update_user',
            options: options
        }; 

        jQuery.post( globalids.url, data, function(response) {
            alert(response);
        });
    }

    // update user field
    $('.js-update-user-field').on('click', function(event){
        event.preventDefault();

        var $input = $(this).closest('.b-cabinet-info__modify').find('input[name]');
        var options = {};

        options[$input.attr('name')] = $input.val();

        updateUserMaster(options);
    });

    $('#image-upload').on('change', function(){
        $('.download-pic__img-aria img').hide();
        upload(function(){
    ...
    // $('.js-update-user-form').on('submit', function(event){
    //     event.preventDefault();
    ...
    ;(function(){
        ...
        var $userPostsType = $('.js-user-posts-type');

        var $userPost = $('.info-materials__preview-post');
        var $userInfoTab = $('.info-materials__tab');

        if(!$.exists($userPostsType)) {
            return;
        }

        $userPostsType.on('click', function(event){
            event.preventDefault();
            ...
            $userPostsType.removeClass('active-info');
        ...
        $.each($userInfoTab, function(i,el){
            var $self = $(this);

            var $posts = $self.find('.info-materials__preview-post'); 
            var $statusTab = $self.find('.info-materials__preview-post-status');

            $posts
                .removeClass('is-hidden')
                .filter('[data-post-status="publish"]')
                .prependTo($statusTab.filter('[data-post-status="publish"]'));

            $posts
                .filter('[data-post-status="pending"]')
                .prependTo($statusTab.filter('[data-post-status="pending"]'));

            var $userPostsStatus = $self.find('.js-user-posts-status');
            var $userPostsTag = $self.find('.js-user-posts-tag');
            var $postStatusTab = $self.find('.info-materials__preview-post-status');       

            $userPostsStatus.on('click', function(event){
                event.preventDefault();

                var postStatus = $(this).data('post-status');

                $userPostsStatus.removeClass('active-info');
                $(this).addClass('active-info');

                $postStatusTab
                    .removeClass('active-info')
                    .hide()
                    .filter('[data-post-status="' + postStatus + '"]')
                    .fadeIn();
            });

            var $postsTag = $self.find('.js-user-posts-tag');

            $postsTag.on('click', function(){

                $(this).toggleClass('active-info');
                var len = $postsTag.filter('.active-info').length;
                ...
                if(len == 2 || len == 0) {
                    ...
                    var dataTag = '[data-tags="' + $postsTag.filter('.active-info').data('tags') + '"]';
                    $posts.filter(dataTag).show();
                }
